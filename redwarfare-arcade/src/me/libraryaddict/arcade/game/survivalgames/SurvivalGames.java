...
public class SurvivalGames extends SoloGame
{
    ...
    public SurvivalGames(ArcadeManager arcadeManager)
        ...
        setOption(GameOption.PLAYER_DEATH_SOUND, Pair.of(Sound.ENTITY_LIGHTNING_BOLT_THUNDER, 2F));
    ...
    public void onSecond(TimeEvent event)
    {
        ...
        if (timeTillDeathmatch > 60)
        {
            ...
            case 0:
                ...
                {
                    player.playSound(player.getEyeLocation(), Sound.ENTITY_IRON_GOLEM_DEATH, 1000.0F, 0.0F);
                }
        ...
        else if (timeTillWarmup == 0)
        {
            for (Player player : UtilPlayer.getPlayers())
            {
                player.playSound(player.getEyeLocation(), Sound.ENTITY_ZOMBIE_ATTACK_WOODEN_DOOR, 1000.0F, 0F);
            }
