...
public class DemolitionsAbility extends Ability
{
    ...
    public void onBlockBreak(BlockBreakEvent event)
    {
        ...
        Iterator<Landmine> itr = _mines.iterator();

        while (itr.hasNext())
        {
            Landmine mine = itr.next();
    ...
    public void onBlockPlace(BlockPlaceEvent event)
    {
        ...
        if (event.getBlock().getRelative(BlockFace.DOWN).getType().toString().startsWith("INFESTED"))
        {
            return;
        }

        for (Barricade barricarde : BarricaderAbility.allBarricades) {
            if (barricarde.getBlock() == event.getBlockAgainst()) {
                player.sendMessage(C.Red + "That is a barricade!");
                return;
            }
        }
    ...
    public void onMineRegen(TimeEvent event)
    {
        ...
        while (itel.hasNext())
        {
            ...
            UtilInv.addItem(player, new ItemBuilder(Material.STONE_PRESSURE_PLATE).setTitle(C.Yellow + "Landmine").build());
