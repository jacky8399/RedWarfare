...
public class ExplosiveAbility extends Ability
{
    ...
    public void onInteract(PlayerInteractEvent event)
    {
        ...
        if (!UtilInv.isItem(itemStack, Material.FIREWORK_STAR))
            return;
        ...
        ItemStack item = new ItemBuilder(Material.FIREWORK_STAR).setColor(getGame().getTeam(player).getColor())
    ...
    public void onTime(TimeEvent event)
    {
        ...
        for (Player player : getPlayers(true))
        {
            ...
            if (UtilInv.count(player, Material.FIREWORK_STAR) >= 20)
                continue;

            Recharge.use(player, "GrenadeGive", 13000);

            UtilInv.addItem(player, new ItemBuilder(Material.FIREWORK_STAR).setTitle(C.Yellow + "Grenade")
