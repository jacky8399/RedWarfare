...
public class SacrificialAbility extends Ability
{
    ...
    public void onDamage(CustomDamageEvent event)
    {
        ...
        ((CraftLivingEntity) dmgd).getHandle().aE = 1.5F;
    ...
    public void onEntityInteract(PlayerInteractEntityEvent event)
    {
        ...
        if (!UtilInv.isHolding(player, event.getHand(), Material.INK_SAC) || player.getInventory().getItemInMainHand().getType().toString().endsWith("DYE"))
            return;
    ...
    public void testBonds(Player player) {

        if (_sacrificed.get(player.getUniqueId()) == null)
            return;

        for (Iterator<UUID> iterator = _sacrificed.get(player.getUniqueId()).iterator(); iterator.hasNext(); ) {
            UUID uuid = iterator.next();
            Player sacraficed = Bukkit.getPlayer(uuid);
            int radius = 15;
            boolean isClose = sacraficed.getLocation().distanceSquared(player.getLocation()) <= radius * radius;

            if (!isClose) {
                if (Recharge.canUse(player, "SacTimeout-" + uuid.toString())) {
                    player.sendMessage(C.Red + "You bond has broken away from " + sacraficed.getName() + "!");
                    iterator.remove();
                    continue;
                }

                System.out.print(Recharge.getTimeLeft(player, "SacTimeout-" + uuid.toString()));
                if (Recharge.getTimeLeft(player, "SacTimeout-" + uuid.toString()) > 1000)
                    player.sendMessage(C.Red + "You have grown far from " + sacraficed.getName() + "! Catch up with them!");
                continue;
            }

            Recharge.use(player, "SacTimeout-" + uuid.toString(), 3000);
