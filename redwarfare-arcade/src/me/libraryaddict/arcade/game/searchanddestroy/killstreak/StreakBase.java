...
public abstract class StreakBase implements Listener
{
    ...
    private void handleLanding(Entity entity)
    {
        ...
        if (getProperBlock())
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                else if (block.getRelative(BlockFace.DOWN).getType().isTransparent())
                {
    ...
    {
        if (block.getType() != Material.AIR || !block.getRelative(BlockFace.DOWN).getType().isTransparent())
        {
            return false;
        }
    ...
    public void onItemUse(PlayerInteractEvent event)
    {
        ...
        net.minecraft.server.v1_15_R1.ItemStack nmsItem = CraftItemStack.asNMSCopy(getItem());
        net.minecraft.server.v1_15_R1.ItemStack nmsPlayerItem = CraftItemStack.asNMSCopy(item);
        ...
        boolean type = item.getType() == getItem().getType() && (!item.getType().toString().endsWith("SPAWN_EGG")
