...
public class SearchAndDestroy extends TeamGame
{
    ...
    private long _deathTimer = 420000;
    private long  _nukeTimer = 300000;
    private KillstreakManager _killstreakManager;
    private long _lastDeath;
    private boolean _isNuke = false;
    ...
    public SearchAndDestroy(ArcadeManager arcadeManager)
    {
        ...
        setKits(new KitTrooper(), new KitBerserker(), new KitDemolitions(), new KitDwarf(), new KitExplosive(),
                ...
                new KitWraith(getManager().getPlugin()), new KitBleeder(), new KitSacrificial(), new KitInertia(),
                new KitNone(), new KitBarricader(), new KitBurst());
    ...
    {
        return UtilTime.elasped(getStateChanged(), _deathTimer);
    }
    ...
    public void onPoison(TimeEvent event)
    {
        ...
        if (!_isNuke && UtilTime.elasped(getStateChanged(), _nukeTimer)) {
            Announce(C.Red + "A nuke is waiting to be armed!");
        ...
        if (_poisonStage == 0 && UtilTime.elasped(getStateChanged(), _deathTimer - 60000))
        {
            _poisonStage++;
        ...
        else if (_poisonStage >= 1 && isEndGame())
        {
            if (_poisonStage == 1)
            {
                ...
                Announce(C.Red + "The poison will stop if a bomb is armed!");
    ...
    public void sendTimeProgress(Player player)
    {
        ...
        if (!isEndGame())
        {
            player.sendMessage(C.Gray + "Poison will begin in " + UtilNumber
                    .getTime((getStateChanged() + _deathTimer) - System.currentTimeMillis(), TimeUnit.MILLISECONDS));
        }

        if (!_isNuke)
        {
            player.sendMessage(C.Gray + "Nuke will appear in " + UtilNumber
                    .getTime((getStateChanged() + _nukeTimer) - System.currentTimeMillis(), TimeUnit.MILLISECONDS));
        }
    }
    ...
    public int getPoisonState() {
        return _poisonStage;
    }

    public void primeNuke() {

        World world = getBombs().get(0).getBlock().getWorld();

        int x = (getData().getMaxX() + getData().getMinX()) / 2;
        int z = (getData().getMaxZ() + getData().getMinZ()) / 2;
        int y;

        for (y = getSpectatorSpawn().getBlockY(); y > 0; y--) {
            System.out.print(world.getBlockAt(x, y - 1, z).getType());
        ...
        TeamBomb teamBomb = new TeamBomb(this, null, world.getBlockAt(x, y, z));

        _bombs.add(teamBomb);
        teamBomb.drawHologram();
    }
}
