...
public class KitSacrificial extends SnDKit
{
    ...
    {
        super("Sacrificial", KitAvailibility.Purchase, new String[]
            {
                    "This kit will lets the user sacrifice their health in place of one of their allies. When being sacrificed for, the ally will transfer all damage taken to the sacrificial, as long as they remain within range, must consume their every replenishing steak to survive. Be cautious, for the sacrificial will take 5x more damage when directly attacked!"
            }, new SacrificialAbility());

        setPrice(600);

        setItems(new ItemBuilder(Material.WOODEN_SWORD).addEnchantment(Enchantment.KNOCKBACK, 1).build(),
                new ItemBuilder(Material.RED_DYE).setTitle(C.Red + "Sacrificial Magic").build(),
