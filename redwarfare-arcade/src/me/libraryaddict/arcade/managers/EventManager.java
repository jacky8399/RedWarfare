...
public class EventManager extends MiniPlugin {
    ...
    public void onFishingRod(ProjectileHitEvent event) {
        ...
        net.minecraft.server.v1_15_R1.EntityFishingHook nms = ((CraftFishHook) event.getEntity()).getHandle();
    ...
    public void onRegeneration(EntityRegainHealthEvent event) {
        if (getGame() instanceof SearchAndDestroy) {
            SearchAndDestroy searchAndDestroy = (SearchAndDestroy) getGame();
            if (searchAndDestroy.getPoisonState() > 1) {
                event.setCancelled(true);
            }
        }
