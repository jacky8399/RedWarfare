...
public class CommandBuildJoinServer extends SimpleCommand {
    private JavaPlugin _plugin;
    private ServerType[] _serverTypes;
    private WorldManager _worldManager;

    public CommandBuildJoinServer(JavaPlugin plugin, WorldManager worldManager) {
        ...
        ServerType[] serverType = new ServerType[]{ServerType.SearchAndDestroy, /*ServerType.SurvivalGames,*/
                ServerType.Hub, ServerType.Build, ServerType.GearWars};
    ...
    private ServerType getServer(String alias) {
        ...
        for (ServerType serverType : _serverTypes) {
            if (!serverType.getName().replaceAll(" ", "").equalsIgnoreCase(alias) && !serverType.getShortened()
                    .replaceAll(" ", "").equalsIgnoreCase(alias))
                continue;
    ...
    public void runCommand(Player player, PlayerRank rank, String alias, String[] args) {
        ...
        if (type == ServerType.Hub) {
            ...
            if (player.getWorld() != world) {
                ...
                player.sendMessage(C.Gold + "To join the main hub, use /hub again");
                return;
            }
        }
