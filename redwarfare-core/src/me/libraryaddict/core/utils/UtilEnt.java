...
public class UtilEnt {
    ...
    public static int getArmorRating(LivingEntity entity) {
        ...
        for (ItemStack itemstack : nms.getArmorItems()) {
            if (itemstack != null && itemstack.getItem() instanceof ItemArmor) {
                int l = ((ItemArmor) itemstack.getItem()).e();
    ...
    public static Pair<Entity, Block> getHit(Entity movingEntity, Predicate<Entity> predicate) {
        net.minecraft.server.v1_15_R1.Entity nms = ((CraftEntity) movingEntity).getHandle();

        Vec3D vec3d = new Vec3D(nms.locX(), nms.locY(), nms.locZ());
        Vec3D vec3d1 = new Vec3D(nms.locX() + nms.getMot().x, nms.locY() + nms.getMot().y, nms.locZ() + nms.getMot().z);
        MovingObjectPosition movingobjectposition = nms.world.rayTrace(new RayTrace(vec3d, vec3d1, RayTrace.BlockCollisionOption.COLLIDER, RayTrace.FluidCollisionOption.ANY, nms));

        vec3d = new Vec3D(nms.locX(), nms.locY(), nms.locZ());
        vec3d1 = new Vec3D(nms.locX() + nms.getMot().x, nms.locY() + nms.getMot().y, nms.locZ() + nms.getMot().z);

        if (movingobjectposition != null) {
            vec3d1 = new Vec3D(movingobjectposition.getPos().x, movingobjectposition.getPos().y, movingobjectposition.getPos().z);
        }

        net.minecraft.server.v1_15_R1.Entity entity = null;
        List list = nms.world.getEntities(nms, nms.getBoundingBox().a(nms.getMot().x, nms.getMot().y, nms.getMot().z).g(1.0D));

        double d5 = 0.0D;

        for (int j = 0; j < list.size(); j++) {
            net.minecraft.server.v1_15_R1.Entity entity1 = (net.minecraft.server.v1_15_R1.Entity) list.get(j);
            ...
            if (predicate != null ? predicate.apply(entity1.getBukkitEntity()) :
               ...
               // AxisAlignedBB axisalignedbb = entity1.getBoundingBox().g(0.30000001192092896D);
                ...
                MovingObjectPosition movingobjectposition1 = entity1.a(1, 1, true);

                if (movingobjectposition1 != null) {
                    double d6 = vec3d.distanceSquared(movingobjectposition1.getPos());
                    if ((d6 < d5) || (d5 == 0.0D)) {
                        entity = entity1;
                        d5 = d6;
                    }
        ...
        Block block = movingobjectposition != null && movingobjectposition.getPos() != null ? movingEntity.getWorld()
                .getBlockAt((int) Math.floor(movingobjectposition.getPos().getX()),(int)  Math.floor(movingobjectposition.getPos().getY()),
                        (int) Math.floor(movingobjectposition.getPos().getZ())) : null;
