...
public class UtilLoc
{
    ...
    public static boolean hasSight(Location from, Location to)
    {
        ...
        {
            if (from.getBlock().getType().isOccluding())
            {
                return false;
            }
    ...
    public static Location getLookingAt(Location fromPoint, Vector vec)
    {
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            if (from.getBlock().getType().isTransparent())
                continue;
    ...
    public static boolean isSafeTeleport(Location loc)
    {
        ...
        for (int y = -1; y <= 1; y++)
        {
            for (BlockFace face : BlockFace.values())
            {
                ...
                if(block.getType().isInteractable() && !block.getType().isOccluding()) {
                    return false;
                }
            }
        }
    ...
    public static boolean isSpawnableHere(Location loc)
    {
        ...
        for (int y = 1; y <= 2; y++)
        {
            if (!b.getRelative(BlockFace.UP, y).getType().isTransparent())
            {
                return false;
            }
        }
