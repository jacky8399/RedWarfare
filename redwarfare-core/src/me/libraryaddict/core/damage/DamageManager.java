...
public class DamageManager extends MiniPlugin
{
            ...
            if (cause instanceof Player && ((Player) cause).isSprinting())
            {
                ...
                event.addRunnable(new DamageRunnable("Sprinting")
                {
                    ...
                    public void run(CustomDamageEvent event2)
                    {
                        ...
                        double motX = player.getMot().x * .6;
                        double motY = player.getMot().y;
                        double motZ = player.getMot().z * .6;
                        player.setMot(new Vec3D(motX, motY, motZ));
                    }
                });
            }
        }
    ...
    private Vector reduceVelocity(LivingEntity entity, double motX, double motY, double motZ)
    {
        ...
        Block block = ((CraftBlock) entity.getLocation().getBlock().getRelative(BlockFace.DOWN)).getNMS().getBlock();
        ...
        if (entity instanceof LivingEntity && ((CraftLivingEntity) entity).getHandle().isClimbing())
        {
