        ...
        new BukkitRunnable()
        {
            public void run()
            {
                ...
                case "favicon":
                    ...
                    String path = _plugin.getServer().getWorldContainer().getAbsolutePath();

                    try {
                        File img = new File(path + File.separator + value);
                        BufferedImage buffImg = ImageIO.read(img);

                        buffImg.getData();
                    } catch (Exception e) {
                        UtilPlayer.sendMessage(player, UtilError.format("Unrecognized image '" + value + "'"));
                        return;
                    }

                    player.sendMessage(C.Blue + path);
                    player.sendMessage(C.Blue + path + File.pathSeparator + value);
                    player.sendMessage(C.Blue + value);
